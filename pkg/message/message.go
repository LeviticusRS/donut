...
// Type declaration for functions that that provide messages of a certain type. The provider can
// give messages that are already created to be reused or create new messages.
...
type Message interface {
    ...
    // Decodes the message from the buffer. It is expected that the buffer contains the number of bytes
    // required to decode the message, if not then this function will return an error. If this
    // function returns no error then decoding the message was successful. Implementations of
    // this method are expected not to validate the decoded data to any contextual source.
    // Exceptions to this include where there are checks to assure that the message is valid.
    ...
    // Encodes the message to a buffer. It is expected that the buffer capacity is enough to contain
    // the entire encoded message, if not then this function will return an error.
