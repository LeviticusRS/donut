...
// indefinitely block as it will cause go routines to leak.
...
// particular message then when a receiver for a message blocks the message may not be published to other receivers that
// accept the message. It is fairly expensive to spin up a go routine per message being sent as there could potentially
// be thousands or tens to hundreds of thousands per minute. This adds up quickly, the scheduler and garbage collector
// would be strained. The alternative to this implementation is to pass the message to a queue where a worker accepts a
